...
| macOS | works, except credentials | Menu bar item, popup, settings, autostart entry, no Dock icon - all seen on one machine. The token lives in the login Keychain there, which ClaudeDial does not read yet, so it needs `CLAUDE_CODE_OAUTH_TOKEN`. No release while that is true. |
