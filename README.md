...
| macOS | works, credentials unverified | Menu bar item, popup, settings, autostart entry, no Dock icon - all seen on one machine. The token lives in the login Keychain there; ClaudeDial reads it through `security`, but that path has never run against a real subscription, so `CLAUDE_CODE_OAUTH_TOKEN` is the supported way in. Shipped as a release asset anyway, arm64 only and ad-hoc signed, because a build somebody can download is the only way that gets tried. |
