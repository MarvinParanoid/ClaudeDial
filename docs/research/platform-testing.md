# Platform testing — how the answers were arrived at

The archaeology behind [platform-support.md](../platform-support.md). That
document says what ClaudeDial does today; this one says what was measured, what
was tried and abandoned, and which conclusions were wrong before they were
right.

It is kept for two reasons. The measurements — 15x15 icon requests, a Plasma
panel at `#202326`, exact XEmbed geometry, an Apple Silicon kernel killing an
unsigned binary — are worth more than any comment in the code, and none of them
can be recovered by reading it. And the wrong turns are where the mechanism
actually lives: three of the conclusions below were confidently stated and then
disproved by a measurement, which is the strongest argument this project has for
measuring rather than reasoning.

Nothing here is a guarantee. For that, read the contract.
...
### What one session on a real machine established
...
### The traps, and which of them are now sprung
